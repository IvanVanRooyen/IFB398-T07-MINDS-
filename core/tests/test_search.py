...
class KeywordSearchTests(TestCase):
    ...
    def test_stemmed_word_match(self):
        # tsvector uses linguistic stemming — "studies" matches "Study" via English stemming
        results = self._get("studies")
        self.assertIn("Uranium Feasibility Study", results)
...
class FullTextSearchTests(TestCase):
    """
    Verify that the tsvector-based full-text search returns correct documents.
    These tests run against a real PostgreSQL database — they will be skipped
    automatically on SQLite (which does not support SearchVectorField).
    """
    ...
    def setUp(self):
        ...
        self.org = make_org("MineCorp")

        self.doc_drill = make_doc(
            "Drilling Summary",
            extracted_text="The borehole intersected a magnetite skarn at 340 metres depth.",
            organisation=self.org,
        )
        self.doc_geo = make_doc(
            "Geophysical Survey",
            extracted_text="Airborne magnetic survey conducted over the northern tenement.",
            organisation=self.org,
        )
    ...
    def test_stemmed_match(self):
        """'drill' should match 'Drilling' via English stemming."""
        results = self._get("drill")
        self.assertIn("Drilling Summary", results)
        self.assertNotIn("Geophysical Survey", results)

    def test_multiword_and_semantics(self):
        """websearch mode ANDs multiple words — both must appear in the document."""
        results = self._get("magnetite skarn")
        self.assertIn("Drilling Summary", results)
        self.assertNotIn("Geophysical Survey", results)

    def test_no_results_for_unknown_term(self):
        results = self._get("xyzzy_no_such_term_999")
        self.assertEqual(results, [])
