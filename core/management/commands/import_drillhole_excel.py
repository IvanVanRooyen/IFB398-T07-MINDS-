...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--process", required=True, help="Exact Process name within that organisation."
        )
        parser.add_argument(
            "--dry-run",
            action="store_true",
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            f"  Survey readings  : +{c['surveys_created']} created, {c['surveys_errors']} errors"
        )
        self.stdout.write(
            f"  Lithology rows   : +{c['litho_created']} created, {c['litho_errors']} errors"
        )
        self.stdout.write(
            f"  Assay rows       : +{c['assay_created']} created, {c['assay_errors']} errors"
        )
